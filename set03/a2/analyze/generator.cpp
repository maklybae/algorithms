...
std::vector<int> ArrayGenerator::GenerateRandomVec() {
  ...
  for (size_t i = 0; i < max_size_; ++i) {
    result.push_back(distr(random_engine_));
  }
...
void ArrayGenerator::InitNearlySorted() {
  ...
  while (swaps_count > 0) {
    size_t i = random_engine_() % max_size_;
    size_t j = random_engine_() % max_size_;
    if (i != j) {
...
ArrayGenerator::ArrayGenerator(int mn, int mx, size_t max_size, double nearly_factor, int seed)
